...
namespace AppStarter.Helpers
{
    ...
    {
        public static void SerializeToFile<T>(this T baseType, string path)
        {
            XmlSerializer xmlSerializer = new XmlSerializer(typeof(T), typeof(T).GetNestedTypes());
            string @string = new FileInfo(path).Directory.ToString();
            if (!Directory.Exists(@string))
                throw new DirectoryNotFoundException(string.Format("Directory \"{0}\" not found", @string.Length > 30 ? (object)(@string.Substring(0, 30) + "...") : (object)@string));
            using (FileStream fileStream = new FileStream(path, FileMode.Create, FileAccess.Write, FileShare.Read))
                xmlSerializer.Serialize((Stream)fileStream, (object)baseType);
        }
        ...
        public static T DeserializeFromFile<T>(string filename)
        {
            ...
            if (!File.Exists(filename))
                throw new FileNotFoundException(string.Format("Directory \"{0}\" not found", filename.Length > 30 ? (object)(filename.Substring(0, 30) + "...") : (object)filename));
            T obj;
            using (FileStream fileStream = new FileStream(filename, FileMode.Open, FileAccess.Read, FileShare.Read))
                obj = (T)xmlSerializer.Deserialize((Stream)fileStream);
            return obj;
        }
        ...
        {
            if ((uint)xml.Length <= 0U)
                return default(T);
            T obj;
            using (Stream streamFromString = IOHelper.GenerateUnicodeStreamFromString(xml))
                obj = (T)new XmlSerializer(typeof(T)).Deserialize(streamFromString);
            return obj;
        }


        // https://docs.microsoft.com/en-us/dotnet/api/system.security.cryptography.xml.encrypteddata?redirectedfrom=MSDN&view=netframework-4.8
